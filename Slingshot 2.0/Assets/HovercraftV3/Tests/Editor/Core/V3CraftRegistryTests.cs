...
namespace Lunarlight.Hovercraft.V3.Tests
{
    public sealed class V3CraftRegistryTests
    {
        ...
        [Test]
        public void AuthoredVariant_DeepCopiesLocalTuningAndSharesProducts()
        {
            ...
            CraftBuildDefinition variantA = null;
            CraftBuildDefinition variantB = null;
            try
            {
                variantA = ScriptableObject.CreateInstance<CraftBuildDefinition>();
                variantB = ScriptableObject.CreateInstance<CraftBuildDefinition>();
                variantA.CopySelectionsFrom(source);
                variantB.CopySelectionsFrom(source);
                variantA.ConfigureAuthoringIdentity(
                    "build.authored.variant_a", "guid-a",
                    V3BuildOwnership.AuthoredVariant, false);
                variantB.ConfigureAuthoringIdentity(
                    "build.authored.variant_b", "guid-b",
                    V3BuildOwnership.AuthoredVariant, false);

                var serializedB = new SerializedObject(variantB);
                serializedB.FindProperty("hoverConfiguration")
                    .FindPropertyRelative("targetHoverHeight").floatValue = 11f;
                serializedB.ApplyModifiedPropertiesWithoutUndo();
                variantB.Installations[0].ConfigureAuthority(
                    0.5f, 0.75f, false, Vector3.right);

                Assert.That(variantA.Chassis, Is.SameAs(variantB.Chassis));
                Assert.That(variantA.HoverConfiguration,
                    Is.Not.SameAs(variantB.HoverConfiguration));
                Assert.That(variantA.HoverConfiguration.TargetHoverHeight,
                    Is.Not.EqualTo(variantB.HoverConfiguration.TargetHoverHeight));
                Assert.That(variantA.Installations[0],
                    Is.Not.SameAs(variantB.Installations[0]));
                Assert.That(variantA.Installations[0].AutomaticAuthorityCap,
                    Is.Not.EqualTo(variantB.Installations[0].AutomaticAuthorityCap));
                Assert.That(variantB.Ownership,
                    Is.EqualTo(V3BuildOwnership.AuthoredVariant));
                Assert.That(variantB.AllowLegacyPipelineFallback, Is.False);
            }
            finally
            {
                if (variantA != null) Object.DestroyImmediate(variantA);
                if (variantB != null) Object.DestroyImmediate(variantB);
            }
        }
