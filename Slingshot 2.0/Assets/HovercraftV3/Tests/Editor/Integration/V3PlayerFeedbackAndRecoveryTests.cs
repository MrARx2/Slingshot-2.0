...
namespace Lunarlight.Hovercraft.V3.Tests
{
    public sealed class V3PlayerFeedbackAndRecoveryTests
    {
        ...
        [TestCase(
            "Assets/HovercraftV3/Generated/ReferenceCrafts/V2Parity/" +
            "Builds/ApexV3_MainGimbalDemo.asset",
            "Apex V3 Vector",
            "Rear-Gimbal Vectoring")]
        [TestCase(
            "Assets/HovercraftV3/Generated/ReferenceCrafts/V2Parity/" +
            "Builds/ApexV3_HoverSpringDemo.asset",
            "Apex V3 Terrain",
        ...
        public void FreeDriveSession_BindsAndResetsReferenceCraft()
        {
            ...
            try
            {
                ...
                V3CraftResetNotification notification = default;
                bool notified = false;
                session.CraftResetPerformed += value =>
                {
                    notification = value;
                    notified = true;
                };

                Assert.That(session.ResetCraft(
                    V3CraftResetReason.TestAutomation,
                    "TEST RESET"), Is.True);
                Assert.That(session.ResetSequence, Is.EqualTo(1));
                Assert.That(notified, Is.True);
                Assert.That(notification.Reason,
                    Is.EqualTo(V3CraftResetReason.TestAutomation));
                ...
                session.ConfigureKillPlaneFromTrackBounds(
                    new Bounds(Vector3.zero, new Vector3(100f, 500f, 100f)),
                    250f);
                Assert.That(session.KillPlaneY,
