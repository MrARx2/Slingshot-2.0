...
namespace Lunarlight.Hovercraft.V3
{
    ...
    public sealed class RuntimeSpringMountInstance : RuntimePartInstance
    {
        ...
        private float effectiveCompressionLimitM;
        private float effectiveExtensionLimitM;
        ...
        public float EffectiveCompressionLimitM =>
            effectiveCompressionLimitM;
        public float EffectiveExtensionLimitM => effectiveExtensionLimitM;
            ...
            string parentSocketId)
        {
            ...
            effectiveCompressionLimitM = SpringDefinition != null
                ? SpringDefinition.CompressionLimitM : 0f;
            effectiveExtensionLimitM = SpringDefinition != null
                ? SpringDefinition.ExtensionLimitM : 0f;

            ResetSpringState();
        }

        public void ConfigureOperationalTravelLimits(
            ...
            float extensionLimitM)
        {
            ...
            if (spring == null)
            {
                effectiveCompressionLimitM = 0f;
                effectiveExtensionLimitM = 0f;
                return;
            }

            effectiveCompressionLimitM = Mathf.Min(
                spring.CompressionLimitM,
                Mathf.Max(0f, compressionLimitM));
            effectiveExtensionLimitM = Mathf.Min(
                spring.ExtensionLimitM,
                Mathf.Max(0f, extensionLimitM));
            ResetSpringState();
        }
            ...
            float deltaTime)
        {
            ...
            float displacement01 = displacementM >= 0f
                ? SafeRatio(displacementM, effectiveCompressionLimitM)
                : -SafeRatio(-displacementM, effectiveExtensionLimitM);
