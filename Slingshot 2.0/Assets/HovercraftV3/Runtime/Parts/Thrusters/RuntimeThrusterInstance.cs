...
namespace Lunarlight.Hovercraft.V3
{
    ...
    public sealed class RuntimeThrusterInstance :
        RuntimePartInstance,
        IV3ForceGeneratorDevice
    {
        private Rigidbody rootRigidbody;
        private RuntimeSpringMountInstance springMount;
        private float spoolOutput;
        private float installationAutomaticAuthorityCap;
        private float installationManualAuthorityCap;
        private bool installationEmergencyAllowed = true;
        ...
        public float MaximumForwardForceN =>
            ThrusterDefinition != null
                ? ThrusterDefinition.MaximumForwardForceN
                : 0f;
        public float ProductNormalAuthorityCap => ThrusterDefinition != null
            ? ThrusterDefinition.NormalOutputMultiplier
            : 0f;
        public float ProductEmergencyAuthorityCap => ThrusterDefinition != null
            ? ThrusterDefinition.OverloadOutputMultiplier
            : 0f;
        public float InstallationAutomaticAuthorityCap =>
            installationAutomaticAuthorityCap > 0f
                ? installationAutomaticAuthorityCap
                : ProductNormalAuthorityCap;
        public float InstallationManualAuthorityCap =>
            installationManualAuthorityCap > 0f
                ? installationManualAuthorityCap
                : ProductEmergencyAuthorityCap;
        public bool InstallationEmergencyAllowed => installationEmergencyAllowed;
        public Vector3 WorldForceOrigin =>
            ThrusterDefinition != null
                ? transform.TransformPoint(
                    ThrusterDefinition.LocalForceOrigin)
                : transform.position;
        public Vector3 WorldForceDirection =>
            ThrusterDefinition != null
                ? transform.TransformDirection(
                    ThrusterDefinition.LocalThrustDirection)
                : transform.forward;
        ...
        public void ConfigureInstallationAuthority(SocketInstallation installation)
        {
            installationAutomaticAuthorityCap = installation != null
                ? installation.AutomaticAuthorityCap
                : 0f;
            installationManualAuthorityCap = installation != null
                ? installation.ManualAuthorityCap
                : 0f;
            installationEmergencyAllowed = installation == null ||
                installation.EmergencyAllowed;
        }
        ...
        private float LimitRequestedOutput(
            ThrusterDefinition thruster,
            float requestedOutput,
            bool emergencyOverload)
        {
            bool emergency = emergencyOverload && installationEmergencyAllowed;
            float maximum = emergency
                ? Mathf.Min(
                    thruster.OverloadOutputMultiplier,
                    InstallationManualAuthorityCap)
                : Mathf.Min(
                    ...
                    InstallationAutomaticAuthorityCap);
            float reverseCap = Mathf.Min(1f, maximum);
            float minimum = thruster.MaximumReverseForceN > 0f
                ? -reverseCap
                : 0f;
            return Mathf.Clamp(
                requestedOutput,
