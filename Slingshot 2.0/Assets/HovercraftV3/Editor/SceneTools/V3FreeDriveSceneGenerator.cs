...
namespace Lunarlight.Hovercraft.V3.Editor
{
    public static class V3FreeDriveSceneGenerator
    {
        public const string FreeDriveRoot =
            "Assets/HovercraftV3/Scenes";
        public const string ScenePath =
            FreeDriveRoot + "/Development/V3_HandlingTrack.unity";
        ...
        private const string MaterialsPath =
            FreeDriveRoot + "/Shared/Materials";
        private static readonly Vector3 SpawnPosition =
            new Vector3(0f, 3.4f, -360f);

        [MenuItem("Tools/Hovercraft V3/Generate V3 Handling Track")]
        public static void Generate()
        {
            ...
            try
            {
                ...
                AssignObject(console, "theme", EnsureTheme());

                V3FreeDriveCamera camera = CreateCamera(session);
                V3CraftTestSpawner spawner =
                    sessionObject.AddComponent<V3CraftTestSpawner>();
                CraftBuildDefinition sceneBuild =
                    AssetDatabase.LoadAssetAtPath<CraftBuildDefinition>(
                        SystemsBuildPath);
                if (sceneBuild == null)
                {
                    sceneBuild = assembler.Build;
                }

                AssignObject(spawner, "selectedBuild", sceneBuild);
                AssignObject(assembler, "build", sceneBuild);
                AssignBool(assembler, "assembleOnStart", false);
                AssignObject(spawner, "assembler", assembler);
                AssignObject(spawner, "inputSource", input);
                AssignObject(spawner, "spawnPoint", spawnObject.transform);
                AssignObject(spawner, "session", session);
                AssignObject(spawner, "cameraRig", camera);
        ...
        {
            EnsureFolder("Assets/HovercraftV3", "Scenes");
            EnsureFolder(FreeDriveRoot, "Development");
            EnsureFolder(FreeDriveRoot, "Shared");
            EnsureFolder(FreeDriveRoot + "/Shared", "Materials");
            EnsureFolder("Assets/HovercraftV3", "Content");
            EnsureFolder("Assets/HovercraftV3/Content", "UIThemes");
        }

        private static V3SystemUIThemeDefinition EnsureTheme()
        {
            const string path =
            ...
            V3SystemUIThemeDefinition theme =
                AssetDatabase.LoadAssetAtPath<
                    V3SystemUIThemeDefinition>(path);
            if (theme == null)
            {
                theme =
                    ScriptableObject.CreateInstance<
                        V3SystemUIThemeDefinition>();
