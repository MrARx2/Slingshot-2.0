    ...
    public static class TrackDebugReportExporter
    {
        ...
        private static void Header(StringBuilder sb, TrackGenerator generator, ResolvedTrackGenerationConfig resolved)
        {
            ...
            if (m != null)
            {
                ...
                sb.AppendLine($"loops {m.LoopCount} corks {m.CorkscrewCount} spirals {m.SpiralCount} halfHelix {m.HalfHelixTurnaroundCount} halfloops {m.HalfLoopCount} jumps {m.JumpCount} pipes {m.FullPipeCount} wallrides {m.WallrideCount} dualQuarters {m.DualRoadQuarterCount}");
        ...
        private static string RingRow(int i, in TrackConnectionFrame f)
        {
            ...
            return $"{i,4} | {f.ArcLength,7:F1} | {f.Position.x,8:F1} {f.Position.y,7:F1} {f.Position.z,8:F1} | {heading,6:F1} {pitch,5:F1} {tilt,5:F1} {f.BankAngle,5:F1} | accV {f.AccumulatedVerticalRotation,5:F0} accR {f.AccumulatedRoadRoll,5:F0} rateR {f.RoadRollRate,5:F2} | {f.LeftWallMultiplier,5:F2} {f.RightWallMultiplier,5:F2} | {f.SideHeight,5:F2} | {f.TurnRounding,4:F2} | {f.LeftOverhang,4:F2} {f.RightOverhang,4:F2} | {f.PipeClosure,4:F2} {f.WallrideMorph,4:F2} | {f.Width,5:F1} | {f.LapProgress:F4}";
        }
